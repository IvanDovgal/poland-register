...
export default class AccountService {
  async register({
    password, firstName, lastName, userName, contactNumber,
  }: {
    password: string, firstName: string, lastName: string, userName: string, contactNumber: string
  }) {
    let error = 'none';
    do {
      const jar = request.jar();
      const page = await request.get({
        url: 'https://online.vfsglobal.com/GlobalAppointment/Account/RegisterUser', jar, transform,
      });
      ...
      const formData = {
        body: image.toString('base64'), method: 'base64', key: apiKey, regsense: 1, json: 1,
      };
